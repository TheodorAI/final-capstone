...
def _get_shared_components(api_key: str):
    ...
    # Use shared loader from kg_loader.py
    base = init_shared(api_key, log_dir=os.path.join(DATA_DIR, "logs_shared"))

    # Extra components needed by run_all only
    hybrid_retriever = HybridRetriever(base["vector_retriever"])
    ...
    if not os.path.exists(TEXTBOOK_DIR):
        if os.path.exists(QUESTION_BANK_PATH):
            try:
                base["kg"].load_question_bank(QUESTION_BANK_PATH)
    ...
    _shared_components = dict(
        ...
        logic_retriever=base["logic_retriever"],
        hybrid_retriever=hybrid_retriever,
        qb_retriever=base["qb_retriever"],
